...
import useHeroBuilderData from "../../context/Hero/useHeroBuilderData";
import useHeroBuilderDispatch from "../../context/Hero/useHeroBuilderDispatch";
...
export default function HeroSection() {
  ...
  const data = useHeroBuilderData();
  const { enabled, variant, entries } = data || {};
  const dispatch = useHeroBuilderDispatch();
  console.log(entries)

  // console.log("Hero Section Data:", data);
  // console.log("Hero Section Dispatch:", dispatch);  

  

  if (data.variant !== "carousel" || entries.length === 1) return (
      <HeroContent data={ entries[0] }/>
   );

  return ( <HeroSectionCarousel data={data} /> )
